...
from sklearn.preprocessing import MinMaxScaler
from deeptendies.bucket.local_bucket import save_data
...
class StockData():
    ...
    def get_cleaned_data(self, categorical_cols=["is_quarter_end"], index_col="t", drop_cols=['s', 'wma'],
    ...
    def get_stock_data(self, stock_name, days, period):
        ...
        if res['s'] == 'ok':
            ...
            raise Exception(
                f'No data found for ticker = {stock_name}, days = {days}, period = {period}! Aborting StockData init')

    def get_stock_data_with_dates(self, stock_name, day_start, day_end):
        import numpy as np
        ...
        # ticker = 'GME'
        ticker = stock_name
        ingested_data = data.DataReader(ticker, 'yahoo', start_date, end_date)
        print(ingested_data)
        print(type(ingested_data))
        print(ingested_data.describe())

        save_data(dataframe=ingested_data,
                  bucket='fs:',
                  topic=ticker,
                  version='final',
    ...
    def add_mvwap_col(self, df, window=100):
        ...
        df[col_name] = avgPriceVol.rolling(window, center=False, min_periods=1).sum() / df.v.rolling(window,
                                                                                                     center=False,
                                                                                                     min_periods=1).sum()
        return df
    ...
    def get_high(self, df_new, days):
        for i in days:
            df_new['next_' + str(i) + '_high'] = df_new['h'].rolling(window=i).max().shift(-i).fillna(
                np.nan)  # TODO: Try out other imputation methods?
    ...
    def get_scaled_df(self, df, test_percentage=0.3, ignore_cols=['day_of_week', 'day_of_year', 'is_quarter_end'],
                      target_col='c'):
    ...
    def get_timeseries_generators(df, test_percentage=0.3, target_col="c", length=100, batch_size=1,
                                  windowed_norm=False, min_max_scaler=False):
